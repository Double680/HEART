...
def init():
    ...
    result_path_h0 = f'./results'
    result_path_h1 = os.path.join(result_path_h0, llm_model.split('/')[-1])
    result_path_h2 = os.path.join(result_path_h1, f'{args.retriever}-{args.aug}-{args.retrieve_k}')
    if args.tabheader:
        result_path_h2 = f'{result_path_h2}-tabheader'
    if args.dev:
        result_path_root = os.path.join(result_path_h2, 'dev')
    else:
        result_path_root = os.path.join(result_path_h2, 'test')
    
    ensure_dirs(result_path_h0, result_path_h1, result_path_h2, result_path_root)
    ...
    agent = ThinkAgent(llm_config, result_path_root, retriever)
